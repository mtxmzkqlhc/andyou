        ...
        <div class="row-fluid sortable">		
            <div class="box span12">
                <div class="box-header" data-original-title>
                	  <h2><i class="halflings-icon align-justify"></i><span class="break"></span>商品分类管理</h2>
...
if($data) {
   foreach($data as $v) {
       ...
       $outStr.='<td>'.$v['name'].'</td>';
       $outStr.='<td rel="'.$v['id'].'">
       <a title="修改" class="btn btn-info editbtnProductCate"><i class="halflings-icon white edit"></i></a>
                ...
                <form id="addform" method="post" action="?">
                    <table>
                        <tbody><tr><td> <table class="item_edit_table"> <tbody>
                         ...
                         <input type="hidden" name="c" value="ProductCate">
...
$('.editbtnProductCate').live('click',function(){
    var id = $(this).parent().attr('rel');
    getdatainfo(id,'ProductCate',function(dat){
        $('#name').val(dat['name']);
    }); 
 });
...
var setDataValue = function(id,col,val){
   ...
   $.ajax({
        type: "POST",
        url: "/?c=ProductCate&a=SetValue",
        data: postArr,
        success:function(){
           document.location=bkUrl?bkUrl:'?c=ProductCate';
        }
   });
}
