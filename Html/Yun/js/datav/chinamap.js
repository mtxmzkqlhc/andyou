...
;(function (name, definition) {
  ...
  Chinamap.prototype.gradColor = function(first, last, k, callback) {
        if(typeof(first) === 'string') {
            first = Raphael.getRGB(first);
        }
        if(typeof(last) === 'string') {
            last = Raphael.getRGB(last);
        }
        var step = {
            r: (last.r - first.r) / k,
            g: (last.g - first.g) / k,
        ...
        var c = {};    //似乎Raphael会缓存getRGB的结果, 所以复制一份以防修改
  ...
  Chinamap.prototype.setSource = function (source) {
    ...
    for (key in source) {
      ...
      if (typeof formatName !== 'undefined') {
        this.sourceData[formatName] = source[key].value;
        this.sourceInfo[formatName] = source[key];
      }
    }
  };
