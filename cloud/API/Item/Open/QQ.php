...
class API_Item_Open_QQ
{
    ...
    /**
     * 获取腾讯微博其他用户详细信息
     * 具体接口：http://wiki.open.qq.com/wiki/website/get_other_info
     * 
     */    
    public static function getWeiboOtherUserInfo($paramArr) {
		$options = array(
			...
			'accessToken'    => '', #用户访问的Access Token
			'name'           => '', #其他用户的账户名
			'fopenid'        => '', #其他用户的openid name和fopenid至少选一个，若同时存在则以name值为主。
			'debug'          => false,
		);
		if (is_array($paramArr))$options = array_merge($options, $paramArr);
		extract($options);
