...
class API_Item_Open_Xinge {
	/**
	 * 初始化一个userid和account对应关系	
	 * token为设备名  一个设备一个token
	 * 没有插一条，有不变
	 */
	public static function setAccessToken($paramArr) {
		$options = array(
				'z_api_id'			=> '',		#ACCESS ID
				'z_api_type'		=> 'xinge',	#推送服务类型
				'z_api_token'		=> '',		#设备唯一标识
				'z_api_secert'		=> '',		#SECRET KEY
				'z_api_account'		=> '',		#根据用户生成的唯一码
				'z_bind_userid'     => '', 		#用户ID
		);
	...
	public static function pushSingleDeviceNotificationIos($paramArr){
		$options = array(
				...
				'userId' 		=> '', 	//userId
				'expireTime'	=> 86400,				//消息离线存储多久，单位为秒，最长存储时间3天。
				'sound'			=> 'beep.wav',			//设置通知声音。
				'sendTime'		=> '',					//消息定时推送时间，格式为year-mon-day hour:min:sec，若小于服务器当前时间则立即推送。选填，默认为空字符串，代表立即推送
				'badge'			=> 1,					//设置角标数值
				'custom'		=> false,				//用户自定义的key-value。
		);
		...
		$acceptTime = new TimeInterval(0, 0, 23, 59);			//表示一个允许推送的时间闭区间，从startHour : startMin到endHour : endMin
