...
*/
class API_Item_Open_Sina
{
    private static $WB_AKEY = "4028615622";
    private static $WB_SKEY = "55d59ce671add97eabccf2c20d167b61";
    private static $conObj = array();
    private static $rdmAccessToken = null; #随便获得一个access token
    private static $accessTokenArr = array();#预防重复获得
    /**
     * 初始化微博的一个链接账户
     */
    private static function init($paramArr) {
		$options = array(
        ...
        if(!isset(self::$conObj[$accessToken])){
            #WB_AKEY为App Key，WB_SKEY为App Secret
            $WB_AKEY     = "4028615622";
            $WB_SKEY     = "55d59ce671add97eabccf2c20d167b61";          

            self::$conObj[$accessToken] = new SaeTClientV2( $WB_AKEY , $WB_SKEY , $accessToken);

        }
        return self::$conObj[$accessToken];
    }
    ...
    private static function getRdmAccessToken() {
        if(self::$rdmAccessToken)return self::$rdmAccessToken;

        $db = API_Db_User::instance();
        self::$rdmAccessToken = $db->getOne("select z_oauth_token from `z_api_token`  where z_api_type='sina' order by z_id desc limit 1");
    ...
    public static function getAccessToken($paramArr) {
        ...
        if(!$zolUid && !$uid)return false;

        if(isset(self::$accessTokenArr[$zolUid]))return self::$accessTokenArr[$zolUid];


        $db = API_Db_User::instance();
        #获得API id
        if($zolUid){
            $apiId = self::getUserApiId(array('zolUid'=>$zolUid));
        }else{
            $apiId = $uid;
        }
        #获得access Token
        self::$accessTokenArr[$zolUid] = $db->getOne("select z_oauth_token from  z_api_token where z_api_id = '{$apiId}' and z_api_type='sina' ");
        
        return self::$accessTokenArr[$zolUid];
    }
    ...
    public static function getUserApiId($paramArr) {
		...
		if (is_array($paramArr))$options = array_merge($options, $paramArr);
		extract($options);
        if(!$zolUid)return false;

        $db = API_Db_User::instance();
        #获得API id
        return $db->getOne("select z_api_id from z_login_api where z_bind_userid = '{$zolUid}' and z_api_type='sina'");
    }
	...
	public static function getUserInfo($paramArr) {
        ...
        if(!$accessToken){
            $accessToken = self::getRdmAccessToken();#随便获得一个token
        }
        $c = self::init(array('accessToken' => $accessToken));
        $info = false;

        if($zolUid){
            $uid        = self::getUserApiId(array('zolUid'=>$zolUid));#ZOL用户ID到新浪ID的转换
        }
        if($uid){
            $info = $c->show_user_by_id($uid);
        }elseif($name){
            $name = mb_convert_encoding($name, "UTF-8","GBK");
            $info = $c->show_user_by_name($name);
        }
        if($info) {
            array_walk_recursive($info, "api_json_convert_encoding_u2g");
            $info['weibo_url'] = "http://weibo.com/{$info['idstr']}";   
        }
        return $info;
     ...
     * 获得用户的好友列表
     */
    public static function getUserFriends($paramArr) {
		$options = array(
			'zolUid'         => 0,  #ZOL用户ID
			'uid'            => 0,  #用户ID
			'name'           => '', #用户名
        ...
        $info = false;
        if($uid){
            $info = $c->friends_by_id($uid, $offset);
        }elseif($name){
            $name = mb_convert_encoding($name, "UTF-8","GBK");
            $info = $c->friends_by_name($name);
        }
        if($info)array_walk_recursive($info, "api_json_convert_encoding_u2g");
        return $info;
     ...
     * 获取在微博关注的ZOL用户
     * @param unknown_type $paramArr
     * @return boolean|Ambigous <multitype:, multitype:string unknown Ambigous <PDOStatement, NULL, string> >
     */
    public static function getWeiboFollow($paramArr)
    {
        $options = array(
                'userid'    => '',
                'off_follow'=> 0  ###过滤ZOL已关注用户
        );
        if (is_array($paramArr))$options = array_merge($options, $paramArr);
        extract($options);
        ...
        $data = self::getUserFriends(array('zolUid'=>$userid));
        if (!$data || !$data['total_number']) {
            return false;
        }
        $list = $data['users'];
        $totalNum = $data['total_number'];
        $totalPage = ceil($totalNum/50);
        for($page=2; $page<=$totalPage; $page++) {
            $cursor = $data['next_cursor'];
            $data   = self::getUserFriends(array('zolUid'=>$userid, 'offset'=>$cursor));
            if (!$data || !$data['total_number']) {
                break;
            }
            $list = array_merge($list, $data['users']);
        }
        $data = array();
        $to_userid = '';
        foreach ($list as $key=>$item) {
            $sid = $item['id'];
            if ($z_userid=self::getUserIdBySid(array('sid'=>$sid))) {
                $to_userid .= $z_userid.',';
                $row = array(
                        'zol_userid'    => $z_userid,
                        'screen_name'   => $item['screen_name'],
                        'profile_image_url' => $item['profile_image_url'],
                        'weibo_url'     => "http://weibo.com/{$sid}"
                        );
                $data[$z_userid] = $row;
            }
        }
        if ($off_follow && $to_userid) {
            $to_userid = rtrim($to_userid, ',');
            $userinfo  = API_Item_User_Base::getUserInfo(array('userid'=>$userid));
            $uid       = $userinfo['sid'];
            $follow_info = API_Item_User_Base::checkFollow(array('uid'=>$uid, 'to_userid'=>$to_userid));
            if ($follow_info) {
                foreach ($follow_info as $key=>$item) {
                    if (in_array($item['code'], array('4009', '4012'))) {
                        unset($data[$key]);
                    }
                }
            }
     ...
     * 获得用户的粉丝列表
     */
    public static function getUserFollowers($paramArr) {
		$options = array(
			'zolUid'         => 0,  #ZOL用户ID
			'uid'            => 0,  #用户ID
			'name'           => '', #用户名
        ...
        $info = false;
        if($uid){
            $info = $c->followers_by_id($uid,$offset,$num);
        }elseif($name){
            $name = mb_convert_encoding($name, "UTF-8","GBK");
            $info = $c->followers_by_name($name,$offset,$num);
        }
        if($info)array_walk_recursive($info, "api_json_convert_encoding_u2g");
        return $info;
     ...
     * 获得当前用户的微博
     */
    public static function getUserTimeLine($paramArr) {
		$options = array(
			'zolUid'         => 0,  #ZOL用户ID
			'uid'            => 0,  #新浪平台用户ID
			'page'           => 1,  #第几页，默认1
			'num'            => 50, #数量
			'sinceId'        => 0,  #ID比since_id大的微博消息（即比since_id发表时间晚的微博消息）
			'maxId'          => 0,    #ID小于或等于max_id的提到当前登录用户微博消息。
			'feature'        => 0,    #类型ID，0：全部、1：原创、2：图片、3：视频、4：音乐，默认为0
			'trimUser'       => 1,    #返回值中user信息开关，0：返回完整的user信息、1：user字段仅返回uid，默认为1。
			'baseApp'        => 0,    #
            'screenname'     => 0,  
		);
		if (is_array($paramArr))$options = array_merge($options, $paramArr);
        ...
        $accessToken = false;
        if($zolUid){            
            $accessToken = self::getAccessToken(array('zolUid'=>$zolUid));
            $uid         = self::getUserApiId(array('zolUid'=>$zolUid));#ZOL用户ID到新浪ID的转换
        }elseif($uid){
            $accessToken = self::getAccessToken(array('uid'=>$uid));
        } elseif ($screenname){
            $accessToken = self::getRdmAccessToken();
        }
        ...
        if(!$c)return false;
        if ($screenname){
            $screenname = mb_convert_encoding($screenname, "UTF-8","GBK");
            $data = $c->user_timeline_by_name($screenname , $page , $num , $sinceId, $maxId, $feature, $trimUser, $baseApp);
        } else {
            $data = $c->user_timeline_by_id( $uid , $page , $num , $sinceId, $maxId, $feature, $trimUser, $baseApp);
        } 
        if(isset($data['error']))return false;
        if($data)array_walk_recursive($data, "api_json_convert_encoding_u2g");
        return $data; 
     ...
     * 获得新浪微博的最新微博
     */
    public static function getPublicTimeLine($paramArr) {
		$options = array(
			'page'           => 1,  #第几页，默认1
			'num'            => 50, #数量
			'baseApp'        => 0,    #
        ...
        $data = $c->public_timeline( $page , $num ,$baseApp);
     ...
     * 获得用户首页的最新微博
     */
    public static function getHomeTimeLine($paramArr) {
		$options = array(
			'zolUid'         => 0,  #ZOL用户ID
			'uid'            => 0,  #新浪平台用户ID
			'page'           => 1,  #第几页，默认1
			...
			'sinceId'        => 0,  #ID比since_id大的微博消息（即比since_id发表时间晚的微博消息）
			'maxId'          => 0,  #ID小于或等于max_id的提到当前登录用户微博消息。
			'feature'         => 0, # 0：全部、1：原创、2：图片、3：视频、4：音乐，默认为0。
			'baseApp'        => 0, 
            'screenname'     => 0,  
		);
		if (is_array($paramArr))$options = array_merge($options, $paramArr);
        ...
        $data = $c->home_timeline( $page , $num, $sinceId, $maxId ,$baseApp ,$feature);
        if(isset($data['error']))return false;
        if($data)array_walk_recursive($data, "api_json_convert_encoding_u2g");
        return $data; 
     ...
     * 获得当前用户的转发的微博
     */
    public static function getRepost($paramArr) {
		$options = array(
			'zolUid'         => 0,  #ZOL用户ID
			'uid'            => 0,  #新浪平台用户ID
			'page'           => 1,  #第几页，默认1
        ...
        $accessToken = self::getAccessToken(array('zolUid'=>$zolUid,'uid'=>$uid));        
        if(!$accessToken)return false;

        $c = self::init(array('accessToken' => $accessToken));
        if(!$c)return false;

        $data = $c->repost_by_me( $page, $num, $sinceId, $maxId );
        if(isset($data['error']))return false;
        if($data)array_walk_recursive($data, "api_json_convert_encoding_u2g");
        return $data;
     ...
     * 获得@当前用户的微博
     */
    public static function getAtmeList($paramArr) {
		$options = array(
			'zolUid'         => 0,  #ZOL用户ID
			'uid'            => 0,  #新浪平台用户ID
			'page'           => 1,  #第几页，默认1
			...
			'sinceId'        => 0,  #ID比since_id大的微博消息（即比since_id发表时间晚的微博消息）
			'maxId'          => 0,  #ID小于或等于max_id的提到当前登录用户微博消息。
			'byAuthor'       => 0,  #作者筛选类型，0：全部、1：我关注的人、2：陌生人，默认为0。
			'bySource'       => 0,  #来源筛选类型，0：全部、1：来自微博、2：来自微群，默认为0。
			'byType'         => 0,  #原创筛选类型，0：全部微博、1：原创的微博，默认为0。
		);
		if (is_array($paramArr))$options = array_merge($options, $paramArr);
		extract($options);
        ...
        $data = $c->mentions( $page, $num, $sinceId, $maxId,$byAuthor,$bySource, $byType);
        if(isset($data['error']))return false;
        if($data)array_walk_recursive($data, "api_json_convert_encoding_u2g");
        return $data;
     ...
     * 获得@当前用户的微博收藏标签
     */
    public static function getUserTags($paramArr) {
		$options = array(
			'zolUid'         => 0,  #ZOL用户ID
			'uid'            => 0,  #新浪平台用户ID
			'page'           => 1,  #第几页，默认1
        ...
        $c = self::init(array('accessToken' => $accessToken));
        if(!$c)return false;
        $data = $c->favorites_tags( $page, $num);
        if(isset($data['error']))return false;
        if($data)array_walk_recursive($data, "api_json_convert_encoding_u2g");
        return $data;
     ...
     * 获得一个微博的评论列表
     */
    public static function getCommentsBySid($paramArr) {
		$options = array(
			'sid'             => 0,  #具体的微博ID
			'sinceId'        => 0,  #返回比sinceid大的评论
			'maxId'          => 0,  #返回比maxId小的评论
			'count'          => 50,  #单页返回的记录条数，默认为50。
			'page'           => 1,  #返回结果的页码，默认为1。
			'filterByAuthor' => 0, #作者筛选类型，0：全部、1：我关注的人、2：陌生人，默认为0。
		);
		if (is_array($paramArr))$options = array_merge($options, $paramArr);
		extract($options);
        ...
        $c = self::init(array('accessToken' => $accessToken));
        $info = $c->get_comments_by_sid( $sid, $page, $count, $sinceId, $maxId,$filterByAuthor );
        if($info)array_walk_recursive($info, "api_json_convert_encoding_u2g");
        return $info;
     ...
     /**
     * 获得微博的操作对象，可执行所有SDK方法
     */
    public static function getWeiboObj($paramArr) {
		$options = array(
			'zolUid'         => 0,  #ZOL用户ID
			'uid'            => 0,  #新浪平台用户ID
		);
     ...
     * @return boolean
     */
    public static function getShortUrl($paramArr)
    {
        $options = array(
            'url'    => ''   #源URL地址
        );
        ...
        $urls = $c -> short_url_shorten(self::$WB_AKEY, array($url));
        $urls = $urls['urls'];
     ...
     * @param unknown_type $paramArr
     */
    public static function getWeiboByUrl($paramArr)
    {
        $options = array(
            'url'    => '',   #源URL地址    
            'num'    => 10,   #返回的条数
            'type'   => 1,    #抓取类型 1-微博 2-评论         
        );
        if (is_array($paramArr))$options = array_merge($options, $paramArr);
        extract($options);
        ...
        $c = self::init(array('accessToken' => $accessToken));
        //根据长链接获取短链接
        $shortUrl = self::getShortUrl(array('url'=>$url));
        if (!$shortUrl) {
            return false;
        }
        $list   = array();
        if ($type==1) {
            $shares = $c -> short_url_weibo(self::$WB_AKEY, $shortUrl, 0, $num);
            if ($shares['share_statuses']) {
                $list = array_merge($list, array_reverse($shares['share_statuses']));    
            }
        }
        if ($type==2) {
            $comment = $c -> short_url_comment(self::$WB_AKEY, $shortUrl, 0, $num);
            if ($comment['share_comments']) {
                $list = array_merge($list, array_reverse($comment['share_comments']));
            }
        }
        ...
        if ($list) {
            foreach ($list as $key=>$item) {
                $mids .= ($key==0) ? '' : ',';
                $mids .= $item['mid']; 
                $data[$key]['id']   = $item['idstr'];  #评论ID
                $data[$key]['text'] = iconv('utf-8', 'gbk', $item['text']); #评论内容
                $data[$key]['time'] = strtotime($item['created_at']); #评论时间
                $data[$key]['picurl']   = $item['pic_urls']=='[]' ? '' : $item['pic_urls']; #内容配图地址
                $data[$key]['sid']      = $item['user']['id']; #发表人ID
                $data[$key]['nickname'] = iconv('utf-8','gbk',$item['user']['screen_name']); #发表人昵称
                $data[$key]['headpic']  = $item['user']['profile_image_url']; #发送人头像 50
                $data[$key]['userurl']  = "http://weibo.com/".$item['user']['profile_url'];  #发送人链接
            }
            if ($type==1) {
                $midArr = $c->querymid($mids, $type, 1);
                $mids   = array();
                foreach ($midArr as $key=>$item) {
                    foreach ($item as $k=>$v) {
                        $mids[$k] = $v;
                    }
                }
                foreach ($data as $key=>$item) {
                    $data[$key]['url'] = "http://weibo.com/{$item['sid']}/{$mids[$item['id']]}";
                }
            }
        } 
        return $data;
    ...
    public static function getWeiboByTopics($paramArr)
    {
        $options = array(
                'q'      => '',   #源URL地址
                'num'    => 10,   #返回的条数
        ...
        if ($list) {
            foreach ($list as $key=>$item) {
                ...
                $data[$key]['text'] = iconv('utf-8', 'gbk', $item['text']); #评论内容
                $data[$key]['time'] = strtotime($item['created_at']); #评论时间
                $data[$key]['picurl']   = isset($item['original_pic']) ? $item['original_pic'] : ''; #内容配图地址
                $data[$key]['sid']      = $item['user']['id']; #发表人ID
                $data[$key]['nickname'] = iconv('utf-8','gbk',$item['user']['screen_name']); #发表人昵称
                $data[$key]['headpic']  = $item['user']['profile_image_url']; #发送人头像 50
                $data[$key]['userurl']  = "http://weibo.com/".$item['user']['profile_url'];  #发送人链接
                $data[$key]['reposts_count'] = $item['reposts_count'];
                $data[$key]['comments_count'] = $item['comments_count'];
                $data[$key]['attitudes_count'] = $item['attitudes_count'];
            }
            $midArr = $c->querymid($mids, 1, 1);
            $mids   = array();
            foreach ($midArr as $key=>$item) {
                foreach ($item as $k=>$v) {
                    $mids[$k] = $v;
...
class SaeTOAuthV2 {
	 ...
	 * @return mixed
	 */
	function getnew($url, $parameters = array()) {
	    $response = $this->oAuthRequest($url, 'GETNEW', $parameters);
	    if ($this->format === 'json' && $this->decode_json) {
	        return json_decode($response, true);
	    }
	    return $response;
	...
	function http($url, $method, $postfields = NULL, $headers = array()) {
		...
		$headers[] = "API-RemoteIP: " . isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : "118.67.127.168";
...
class SaeTClientV2
{
	 ...
	 * @return array
	 */
	function domain_show( $domain, $source='' )
	{
		$params = array();
    ...
    function search_topics( $q,  $count = 10)
	{
		$params = array();
		$params['q'] = $q;
		$params['count'] = $count;
	...
	function short_url_comment($source, $url_short, $since_id=0, $count=200)
	{
	    $params = array(
	            'source'    => $source,
	            'url_short' => $url_short,
	    ...
	    return $this->oauth->get( 'short_url/comment/comments', $params);
	}
	
	function short_url_shorten($source, $long)
	{
	    $long = array_slice($long, 0, 20);
	    $params = join('&', array_map(create_function('$a', '$url = urlencode($a);return "url_long=$url";'), $long))."&source=$source";
	    return $this->oauth->getnew( 'short_url/shorten.json?'.$params);
	}
	
	function short_url_comment_counts($source, $shorts)
	{
	    $shorts = array_slice($shorts, 0, 20);
	    $params = join('&', array_map(create_function('$a', '$url = urlencode($a);return "url_short=$url";'), $shorts))."&source=$source";
	    return $this->oauth->getnew( 'short_url/comment/counts.json?'.$params);
	}
	
	function short_url_share_counts($source, $shorts)
	{
	    $shorts = array_slice($shorts, 0, 20);
	    $params = join('&', array_map(create_function('$a', '$url = urlencode($a);return "url_short=$url";'), $shorts))."&source=$source";
	    return $this->oauth->getnew( 'short_url/share/counts.json?'.$params);
	}
	
	function short_url_weibo($source, $url_short, $since_id=0, $count=200)
	{
	    $params = array(
	            'source'    => $source,
	            'url_short' => $url_short,
	    ...
	    return $this->oauth->get( 'short_url/share/statuses', $params);
	}
