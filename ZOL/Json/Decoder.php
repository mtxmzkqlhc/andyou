...
class ZOL_Json_Decoder
{
     ...
     * {@link self::TYPE_ARRAY} and {@link self::TYPE_OBJECT} for
     * valid values
     * @return void
     */
    protected function __construct($source, $decodeType)
    {
        ...
        if (!in_array($decodeType, array(self::TYPE_ARRAY, self::TYPE_OBJECT)))
        {
            $decodeType = self::TYPE_ARRAY;
        }
     ...
     * return a StdClass object instead, pass {@link self::TYPE_OBJECT} to
     * the $objectDecodeType parameter.
     *
     * Throws a ZOL_Exception if the source string is null.
     *
     * @static
     * @access public
     * @param string $source String to be decoded
     * @param int $objectDecodeType How objects should be decoded; should be
     * either or {@link self::TYPE_ARRAY} or
     * {@link self::TYPE_OBJECT}; defaults to TYPE_ARRAY
     * @return mixed
     * @throws ZOL_Exception
     */
    public static function decode($source = null, $objectDecodeType = self::TYPE_ARRAY)
    {
        if (null === $source) {
            throw new ZOL_Exception('Must specify JSON encoded source for decoding');
        } elseif (!is_string($source)) {
            throw new ZOL_Exception('Can only decode JSON encoded strings');
        }
    ...
    protected function _decodeObject()
    {
        ...
        while ($tok && $tok != self::RBRACE) {
            if ($tok != self::DATUM || ! is_string($this->_tokenValue)) {
                throw new ZOL_Exception('Missing key in object encoding: ' . $this->_source);
            }
            ...
            if ($tok != self::COLON) {
                throw new ZOL_Exception('Missing ":" in object encoding: ' . $this->_source);
            }
    ...
    protected function _decodeArray()
    {
        ...
        while ($tok && $tok != self::RBRACKET) {
            ...
            if ($tok != self::COMMA) {
                throw new ZOL_Exception('Missing "," in array encoding: ' . $this->_source);
            }
    ...
    protected function _getNextToken()
    {
                    ...
                    if ($chr == '\\') {
                        ...
                        switch ($chr) {
                            ...
                            case '\'' :
                                ...
                                break;
                            default:
                                throw new ZOL_Exception("Illegal escape "
                                    .  "sequence '" . $chr . "'");
                            }
                    } elseif ($chr == '"') {
                        break;
        ...
        if ($chr == '-' || $chr == '.' || ($chr >= '0' && $chr <= '9')) {
            if (preg_match('/-?([0-9])*(\.[0-9]*)?((e|E)((-|\+)?)[0-9]+)?/s',
                ...
                if (is_numeric($datum)) {
                    if (preg_match('/^0\d+$/', $datum)) {
                        throw new ZOL_Exception("Octal notation not supported by JSON (value: $datum)");
                    } else {
                        $val  = intval($datum);
                        $fVal = floatval($datum);
                    ...
                    throw new ZOL_Exception("Illegal number format: $datum");
                }
            ...
            throw new ZOL_Exception('Illegal Token');
        }
